...
#else
    const char* pszModule = "futurecoin";
#endif
    if (pex)
        return strprintf(
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "futurecoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "futurecoind.pid"));
